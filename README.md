...
| `!leave` / `!выйди` | Выйти |
| `!mode` | Показать текущий режим реакции и доступные |
| `!mode all` | Отвечать на все фразы |
| `!mode name` | Только когда обращаются по имени ("эй бот"). После ответа — 10 секунд follow-up без имени |
| `!mode listener 0.15` | Слушать разговор, вставлять с шансом 15% |
| `!persona` | Показать персон |
| `!persona toxic` | Токсичный циник (другие: `default/sarcastic/calm/hyper/angry`) |
| `!clear` | Очистить краткосрочную историю |
| `!remember <факт>` | Сохранить факт в долгосрочную память |
| `!forget` | Очистить долгосрочную память |
| `!status` | Состояние бота (режим, персона, история) |
